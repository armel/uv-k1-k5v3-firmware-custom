...
void DOPPLER_ExitMode(void)
{
    ...
    // Restore the receiving frequency
    if (gTxVfo->freq_config_RX.Frequency != gDopplerSavedRxFreq)
    {
...
static void DOPPLER_RenderFrequency(const uint32_t Freq10Hz, const uint8_t Line)
{
    ...
    UI_DisplayFrequency(Buffer, 64, Line * 8, true);
}
...
void DOPPLER_Render(void)
{
    ...
    {
        UI_PrintString(gDopplerState == DOPPLER_STATE_INPUT_DATE ? "DATE" : "TIME", 0, 127, 1, 2);
        ...
        UI_PrintString(gDopplerInputStr, 16, 127, 5, 2);
        return;
    }
    ...
    if (!DOPPLER_HasData())
    {
        UI_PrintString("NO DATA", 0, 127, 1, 2);
    ...
    if (gDopplerPassed)
    {
        UI_PrintString("PASSED", 0, 127, 1, 2);
    }
